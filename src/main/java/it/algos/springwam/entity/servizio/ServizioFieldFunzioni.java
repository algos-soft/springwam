...
public class ServizioFieldFunzioni extends AField {
    ...
    public void creaContent() {
        ...
        Grid.Column colonnaSigla = grid.addComponentColumn(funzione -> {
            ...
            if (((Funzione) funzione).isObbligatoria()) {
                labelTxt = LibText.setRossoBold(sigla);
            } else {
                labelTxt = LibText.setBluBold(sigla);
        ...
        Grid.Column colonnaDescrizione = grid.addComponentColumn(funzione -> {
            ...
            String labelTxt = "";
            if (((Funzione) funzione).isObbligatoria()) {
                labelTxt = LibText.setRossoBold(descrizione);
            } else {
                labelTxt = LibText.setBluBold(descrizione);
            }// end of if/else cycle
            label.setValue(labelTxt);
        ...
        Grid.Column colonnaCheck = grid.addComponentColumn(funzione -> {
            ...
            if (LibText.isEmpty(((Funzione) funzione).getId())) {
            } else {
        ...
        colonnaCheck.setWidth(LibColumn.WIDTH_CHECK_BOX);
        ...
        grid.setWidth(lar + width + LibColumn.WIDTH_CHECK_BOX+565, Unit.PIXELS);
    ...
    public Component initContent() {
        ...
        if (entityBean != null) {
            ...
            if (grid != null && funzioni != null) {
                ...
                if (LibText.isValid(lastFunz.getId())) {
    ...
    private void codeChanged(String idKey, String oldCode, String newCode) {
        ...
        if (LibText.isEmpty(oldCode) && LibText.isValid(newCode)) {
            aggiungeRiga(newCode);
            return;
        }// end of if cycle

        //--modifica una riga
        if (LibText.isValid(oldCode) && LibText.isValid(newCode)) {
    ...
    private void aggiungeRiga(String newCode) {
        Funzione funz = funzioneService.findByCode(newCode);
    ...
    private void modificaRiga(String idKey, String newCode) {
        ...
        newFunz = funzioneService.findByCode(newCode);
